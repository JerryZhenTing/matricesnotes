...
\ex{}{Binomial MGF $Y \sim$ bin$(n,0)$ 
	\begin{align}
		m_y(t) = E(e^{tY}) &= \sum_{y=0}^{n} {n \choose y}p^{y}(1-p)^{n-y}e^{ty}
		\\
		&= \sum_{y=0}^{n} {n \choose y}(pe^{t})^{y}(1-p)^{n-y} \\
		&= (pe^{t}+1-p)^n
	\end{align}
	}
...
\thm{}{Let $Y_1,Y_2,\dots,Y_n$ be a random sample of size n from a normal distribution
with mean $\mu$ and variance $\sigma^2$, then $$\bar{Y} = \frac{1}{n}\sum_{i=1}^{n}Y_i$$ is normally 
distributed with mean $\mu_{\bar{Y} = \mu}$ and variance $\sigma_{\bar{Y}}^{2} = \frac{\sigma^2}{n}$}
then $$Z = \frac{\bar{Y}-\mu_{\bar{Y}}}{\sigma_{\bar{Y}}} = \frac{\bar{Y}-\mu}{\frac{\sigma}{\sqrt{n}}} = \sqrt{n}(\frac{\bar{Y}-\mu}{\sigma})$$
\thm{}{
Let $Y_1, Y_2, \ldots, Y_n$ be defined as in Theorem 7.1. Then $Z_i = \frac{(Y_i - \mu)}{\sigma}$ are independent, standard normal random variables, $i = 1, 2, \ldots, n$, and
\[
...
\thm{}{
	Let $Z$ be a standard normal random variable and let $W$ be a $\chi^2$-distributed variable with $\nu$ degrees of freedom (df). Then, if $Z$ and $W$ are independent,
\[
T = \frac{Z}{\sqrt{W / \nu}}
\]
is said to have a \textit{t distribution} with $\nu$ df.
}
If $Y_1, Y_2, \ldots, Y_n$ constitute a random sample from a normal population with mean $\mu$ and variance $\sigma^2$, Theorem 7.1 may be applied to show $Z = \sqrt{n} \frac{\overline{Y} - \mu}{\sigma}$ has a standard normal distribution. Theorem 7.3 tells us that $W = (n-1)\frac{S^2}{\sigma^2}$ has a $\chi^2$ distribution with $\nu = n - 1$ df and that $Z$ and $W$ are independent (because $\overline{Y}$ and $S^2$ are independent). Therefore, by Definition 7.2,
\[
T = \frac{Z}{\sqrt{W / \nu}} = \frac{\sqrt{n} \frac{\overline{Y} - \mu}{\sigma}}{\sqrt{[(n-1)S^2 / \sigma^2] / (n-1)}} = \sqrt{n} \frac{\overline{Y} - \mu}{S}
\]
has a \textit{t distribution} with $(n-1)$ df.
\section{F distribution}
\dfn{}{if $W_1 \sim \chi^2(\nu_1), W_2 \sim \chi^2(\nu_2)$ are independent, then  $$\frac{W_1/\nu_1}{W_2/\nu_2}$$ is said 
to have Snedecor's F-distribution with $(\nu_1,\nu_2)$ degrees of freedom}
$$W_1 = \frac{(n-1)S_1^2}{\sigma_1^2} \sim \chi^2(n-1) \quad W_2 = \frac{(m-1)S_2^2}{\sigma_2^2}\sim \chi^2(m-1)$$
$$\frac{S_1^2 / \sigma_1^2}{S_2^2/\sigma_2^2} \sim F(n-1,m-1)$$
\section{Central Limit Theorem}
\thm{\textbf{Central Limit Theorem}}{
Let $Y_1, Y_2, \ldots, Y_n$ be independent and identically distributed random variables with $E(Y_i) = \mu$ and $V(Y_i) = \sigma^2 < \infty$. Define
\[
U_n = \frac{\sum_{i=1}^n Y_i - n\mu}{\sigma \sqrt{n}} = \frac{\overline{Y} - \mu}{\sigma / \sqrt{n}} \quad \text{where} \quad \overline{Y} = \frac{1}{n} \sum_{i=1}^n Y_i.
\]
...
\nt{The $k^{\text{th}}$ order statistics of a observed sample is the sample's $k^{\text{th}}$ smallest value} 
...
\end{cases}
\]
\nt{\textbf{Special Cases}:
$F_{(n)}(y) = \mathbb{P}(Y_{(n)} \leq y) = (F_Y(y))^n$\\
$f_{(n)}(y) = nF_{Y}(y)^{n-1}f_Y(y)$\\
$F_{(1)}(y) = \mathbb{P}(Y_{(1)} \leq y) = 1 - \mathbb{P}(Y_{(1)} > y) = 1 - (1-F_{Y}(y))^{n}$
\\
